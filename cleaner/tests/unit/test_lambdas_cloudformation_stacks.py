...
def test_lambdas_cloudformation_stacks_get_time_to_live_hours_tag_or_none(raw_stacks):
    ...
    assert get_time_to_live_hours_tag_or_none(raw_stacks[0]) == {'Key': 'time-to-live-hours', 'Value': '72'}
...
def test_lambdas_cloudformation_stacks_get_stack_name(raw_stacks):
    ...
    assert get_stack_name(raw_stacks[0]) == 'igvf-ui-IGVF-246-remove-uuid-as-unique-key-for-treatments-DeployDevelopment-FrontendStack'
...
def test_lambdas_cloudformation_stacks_time_to_live_hours_exceeded(mocker):
    ...
    patched_current_time.return_value = datetime.datetime(2022, 8, 29, 21, 44, 28, 625000, tzinfo=tzutc())
    creation_time = datetime.datetime(2022, 8, 28, 21, 44, 28, 625000, tzinfo=tzutc())
